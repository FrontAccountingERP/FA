...
$sql = "SELECT
	...
	FROM ".TB_PREF."cust_branch b
		...
		LEFT JOIN ".TB_PREF."crm_persons p
			on c.person_id=p.id
	WHERE b.debtor_no = ".db_escape($_GET["cbo_customer_id"])."
		AND b.br_name LIKE " . db_escape("%" . get_post("branch"). "%") . "
	ORDER BY b.br_name LIMIT 0, 10"; // We only display 10 items.
...
while ($myrow = db_fetch_assoc($result))
{
	...
	ahref_cell(_("Select"), 'javascript:void(0)', '', 'selectComboItem(window.opener.document, &quot;' . $_GET["client_id"] . '&quot;, &quot;' . $myrow["branch_code"] . '&quot;, &quot;&quot;, false)');
