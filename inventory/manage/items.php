...
function clear_data()
{
	...
	unset($_POST['no_sale']);
	// Clear BOM session after clearing data.
	clear_bom();
}
...
if (isset($_POST['addupdate']))
{
	...
	if ($input_error != 1)
	{
		...
		{ //it is a NEW part
			...
			// Save BOM for new item if any.
			save_bom($_POST['NewStockID']);
...
function item_settings(&$stock_id)
{
  ...
  // When editing manufactured item, provide the link to open popup window for managing its bill of materials.
	if (is_manufactured($_POST['mb_flag'])){
		$link = $path_to_root . '/manufacturing/manage/bom_edit.php?popup=1&use_session=1';
		if (!$new_item) {
			$link = $path_to_root . '/manufacturing/manage/bom_edit.php?popup=1&stock_id=' . get_post('NewStockID');
		}
		label_row(_("Bills of Material:"), '<a href="' . $link . '" onclick="openWindow(this.href, this.title); return false;" target="_blank">' . _("Manage Bills of Material") . '</a>');
	}
...
function save_bom($parent) {
	// Save BOM from session to database.
	if (isset($_SESSION['bom_session']['list'])) {
		foreach ($_SESSION['bom_session']['list'] as $myrow) {
			add_bom($parent, $myrow['component'], $myrow['workcentre_added'],
				$myrow['location'], $myrow['quantity']);
		}
	}
	// Clear it after that.
	clear_bom();
